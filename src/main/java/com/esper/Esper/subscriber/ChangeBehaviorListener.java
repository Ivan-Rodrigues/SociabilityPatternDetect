package com.esper.Esper.subscriber;

import com.esper.Esper.enums.WeekEnum;
import com.esper.Esper.handler.EventHandler;
import com.esper.Esper.model.NotifyChangeBehavior;
import com.esper.Esper.model.NotifyNewPattern;
import com.esper.Esper.util.EventGenerator;
import com.esper.Esper.util.ExtractPatternUtil;
import com.esper.Esper.util.FileUtil;
...
public class ChangeBehaviorListener implements UpdateListener {
    ...
    public void update(EventBean[] newData, EventBean[] oldData) {
        ...
        try {
            sociabilityPatterns = ExtractPatternUtil.extractPattern(newData, numObs,ParametersUtil.MIN_SUPP,
                    ParametersUtil.NUM_SLOTS, ParametersUtil.PARAM_SLOT,false);
        }catch (Exception e){
            e.printStackTrace();
        }
        ...
        if (countPattern <2){
            ...
            try {
                EventGenerator.client.publish(topic+"/Pattern", new MqttMessage(gson.toJson(notifyNewPattern).getBytes()));
    ...
    private void detectDrift(List<List<Integer>> sociabilityPatterns){
        // Load from 'FCL' file
        String fileName = "fcls\\tipper.fcl";
        ...
        try {
            ...
            EventGenerator.client.publish(topic+"/Change",new MqttMessage(jsonChangeNotify.getBytes()));
